...
//         console.log("reached health checker input")
  ...
  const handleButtonClick = async () => {
    if (isButtonDisabled) {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:${process.env.REACT_APP_API_PORT}/health-checker`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
          },
          body: JSON.stringify({ sources: selectedSources, functions: selectedTestcase }),
        });

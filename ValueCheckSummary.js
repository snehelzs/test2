...
export default function HealthSummary() {
  ...
  const resultsummary = useSelector(state => state.tab.valueCheckerResult)
  ...
  React.useEffect(() => {
    ...
    setKpiData(calculateMetrics(sortedData
      .filter(row => (dataSourceFilter ? row.Measure.includes(dataSourceFilter) : true) &&
                     (testCaseTypeFilter ? row.Attributes.includes(testCaseTypeFilter) : true) &&
  ...
  const handleViewDetails = (details, testType, testId) => {
    console.log("print test typ", testType)
                ...
                <TableBody>
                  {sortedData
                    .filter(row => (dataSourceFilter ? row.data_source.includes(dataSourceFilter) : true) &&
                                   ...
                                   (resultFilter ? row.Result === resultFilter : true))
                    .map((row) => (
                      <TableRow key={row['Test Case Id']}>
                        <StyledTableCell>{row['Test Case Id']}</StyledTableCell>
                        <StyledTableCell>{row['DVE Run Date']}</StyledTableCell>
                        <StyledTableCell>{row.Measure}</StyledTableCell>
                        <StyledTableCell>{row.Attributes}</StyledTableCell>
                        <StyledTableCell>{row.Description}</StyledTableCell>
                        <StyledTableCell>{row['Number of Errors']}</StyledTableCell>   
                        <ResultCell result={row.Result}>{row.Result}</ResultCell>
                        <StyledTableCell>
                          <IconButton onClick={() => handleViewDetails(row.Details, row.Attributes, row['Test Case Id'])} color="primary">
                            <VisibilityIcon />
                          </IconButton>
                        </StyledTableCell>
                      </TableRow>
